...
public class GeoServerRESTImageMosaicManagerTest extends GeoserverRESTTest {
  ...
  /**
   * This test is ignored because it cannot be repeated. Once the test is run, the index is created for the granules, but it won't be removed, so next run will still find all the granules in the db.
   * 
   * Tried with purge=metadata when removing datastores, but removing indices in shapefiles is not supported by geoserver (which throws an UnsupportedOperation) Adding a datastore.properties for using an h2 DB won't work as well, because only postgis is supported at lower levels.
   * 
   * @throws Exception
   */
  @Ignore
  ...
  public void createAndDelete() throws Exception {
    ...
    // crea the manager
    GeoServerRESTStructuredGridCoverageReaderManager manager = new GeoServerRESTStructuredGridCoverageReaderManager(new URL(RESTURL), RESTUSER, RESTPW);
    ...
    Set<String> allowedDateFormats = new HashSet<String>(Arrays.asList("java.util.Date", // shapefile
   ...
   *          the coverage store name
   * @param csname
   *          the coverage name
   */
  
  private void fixDimensions(String wsName, String coverageStoreName, String csname) {
    final GSImageMosaicEncoder coverageEncoder = copyParameters(wsName, coverageStoreName, csname);
   ...
   */
  private GSImageMosaicEncoder copyParameters(String wsName, String coverageStoreName, String csname) throws NumberFormatException {
