...
/**
 * 
 * Note on adding multiple available styles to the GSLayerEncoder: - to run the testIntegration(), 2 clones of the "point" style, named "point2" and "point3" have to be created.
 * 
...
public class GSFeatureEncoderTest extends GeoserverRESTTest {
  ...
  public void testIntegration() throws IOException {
    ...
    // metadataLink
    GSMetadataLinkInfoEncoder metadatalink = new GSMetadataLinkInfoEncoder("text/xml", "ISO19115:2003", "http://www.organization.org/metadata1");
    ...
    // authorityURL
    GSAuthorityURLInfoEncoder authorityURL = new GSAuthorityURLInfoEncoder("authority1", "http://www.authority1.org");
    layerEncoder.addAuthorityURL(authorityURL);
    
    // identifier
    GSIdentifierInfoEncoder identifier1 = new GSIdentifierInfoEncoder("authority1", "identifier1");
    GSIdentifierInfoEncoder identifier2 = new GSIdentifierInfoEncoder("authority1", "another_identifier");
  ...
  public void testFeatureTypeEncoder() {
    ...
    assertNull(el);
    if (el == null) LOGGER.info("REMOVED");
    
    if (LOGGER.isInfoEnabled()) LOGGER.info(encoder.toString());
   ...
   * 
   * Settings information for integration tests - test is based on the data used in http://docs.geoserver.org/latest/en/user/data/database/sqlview.html#parameterizing-sql-views (states shapefile - available in testdata/states.zip) - create a postgis db - import the states shapefile (using shp2pgsql or Postgis shapefile uploader) - In Geoserver, create a postgis datastore for this DB, with the name "statesdb"
   * 
   * @throws IOException
   * 
   */
